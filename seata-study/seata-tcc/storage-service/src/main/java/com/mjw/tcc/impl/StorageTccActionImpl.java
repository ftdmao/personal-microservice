...
import com.mjw.domain.Storage;
import com.mjw.mapper.cutom.MyStorageMapper;
...
public class StorageTccActionImpl implements StorageTccAction {
    ...
    @Autowired
    MyStorageMapper myStorageMapper;
    ...
    @Override
    public boolean prepareDecreaseStorage(BusinessActionContext businessActionContext, Integer productId, Integer count) {
//        log.info("减少商品库存，第一阶段，锁定减少的库存量，productId=" + productId + "， count=" + count);
        ...
        Storage storage = storageMapper.selectOneByExample(example);
         ...
         */
        myStorageMapper.updateFrozen(productId, storage.getResidue() - count, storage.getFrozen() + count);
        //保存标识
...
//        log.info("减少商品库存，第二阶段提交，productId=" + productId + "， count=" + count);
        ...
        myStorageMapper.updateFrozenToUsed(productId, count);
        //删除标识
...
//        log.info("减少商品库存，第二阶段，回滚，productId=" + productId + "， count=" + count);
        ...
        myStorageMapper.updateFrozenToResidue(productId, count);
        //删除标识
        ResultHolder.removeResult(getClass(), businessActionContext.getXid());
        return true;
    }
